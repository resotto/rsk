...
func main() {
	...
	clis = make([]*kgo.Client, (1 << 4))
	for i := 1; i < (1 << 4); i++ {
		cli, err := kgo.NewClient(
			kgo.SeedBrokers(seeds...),
			kgo.ConsumerGroup("score-consumer-group"),
			kgo.ConsumeTopics(scoreTopic+strconv.Itoa(i)),
	...
	for i := 1; i < (1 << 4); i++ {
		...
		eg.Go(func() error { return subscribe(ctx, i) })
	}
...
func subscribe(ctx context.Context, i int) error {
	for {
		select {
		case <-ctx.Done():
			return ctx.Err()
		default:
			fetches := clis[i].PollFetches(ctx)
			...
			for !iter.Done() {
				record := iter.Next()
				s, err := deserialize(string(record.Value))
				if err != nil {
	...
	for {
		...
		case <-ticker.C:
			...
			if probabilities[p] <= ability+rand.Intn(hundredPercent) {
				...
				} else if err := produce(ctx, score.Point, message); err != nil && err != context.DeadlineExceeded {
...
func produce(ctx context.Context, i int, value string) error {
	var res error
	record := &kgo.Record{Topic: scoreTopic + strconv.Itoa(i), Value: []byte(value)}
	clis[i].Produce(ctx, record, func(_ *kgo.Record, err error) {
		res = err
	})
	return res
}
