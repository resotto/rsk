...
var (
	cluster                                               bool
	semaphore, participants, funds, volatility, durations int

	orderClient, priceClient *kgo.Client
	traders                  []*Trader
	r                        *rand.Rand
)
...
func getClient(seeds []string, topic string) *kgo.Client {
...
func main() {
	...
	orderClient = getClient(seeds, orderChannel)
	defer orderClient.Close()

	priceClient = getClient(seeds, currentPriceChannel)
	...
	deleteTopics(orderClient, priceClient)
}

func deleteTopics(clients ...*kgo.Client) {
	topics := []string{orderChannel, currentPriceChannel}
	for i, v := range clients {
		adm := kadm.NewClient(v)
		if _, err := adm.DeleteTopics(context.Background(), topics[i]); err != nil {
...
func processOrders(ctx context.Context, orderBook *OrderBook) error {
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			messages := subscribe(ctx, orderClient)
			for _, m := range messages {
				if order := getResource[Order](m); order != nil {
...
func updateTradersCurrentPrice(ctx context.Context, i int) error {
	for {
		...
		case <-ctx.Done():
			return ctx.Err()
		default:
			messages := subscribe(ctx, priceClient)
			for _, m := range messages {
				if currentPrice := getResource[CurrentPrice](m); currentPrice != nil {
...
func subscribe(ctx context.Context, client *kgo.Client) [][]byte {
	res := make([][]byte, 0)
...
func trade(ctx context.Context, i int) error {
	for {
		...
		case <-ctx.Done():
			...
			return ctx.Err()
		default:
			if r.Intn(hundredPercent) < orderProbability+volatility {
				if order := traders[i].prepareOrder(); order != nil {
					publish(ctx, orderChannel, order)
				}
			}
			if r.Intn(hundredPercent) < alertProbability {
...
func getResource[T resource](message []byte) *T {
	resource, err := deserialize[T](message)
	if err != nil {
...
func publish(ctx context.Context, channel string, a resource) {
	...
	} else if err := produce(ctx, channel, message); err != nil && err != context.DeadlineExceeded {
		fmt.Println("failed to publish", a.string(), err.Error())
	}
}

func produce(ctx context.Context, topic string, value []byte) error {
	var res error
	record := &kgo.Record{Topic: topic, Value: value}
	if topic == orderChannel {
		orderClient.Produce(ctx, record, func(_ *kgo.Record, err error) {
			res = err
		})
	} else if topic == currentPriceChannel {
		priceClient.Produce(ctx, record, func(_ *kgo.Record, err error) {
			res = err
		})
	}
	return res
...
func (t *Trader) getOffsetPrice(forAsk bool) *int {
	targetPrice := t.CurrentPrice.Price - r.Intn(halfPriceRange)
	if forAsk {
		targetPrice = t.CurrentPrice.Price + r.Intn(halfPriceRange)
	}
	return &targetPrice
...
func (t *Trader) prepareOrder() *Order {
	...
	if 0 < t.openOrderCount {
		...
		orderEvent := OrderEvent(1 + r.Intn(2))
		...
		return t.newOrder(uuid.NewString(), Create, orderType, Ask, targetPrice, 1+r.Intn(t.Shares))
	}
...
func newOrderType() OrderType {
	if r.Intn(hundredPercent) < marketProbability {
		return Market
	}
	return Limit
}
